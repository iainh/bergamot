//! Benchmark: nzbg-par2 vs par2cmdline
//!
...
fn bench_verify(dir: &Path, iterations: u32) {
    ...
    for _ in 0..iterations {
        let start = Instant::now();
        let result = nzbg_par2::verify_recovery_set(&rs, dir);
        let elapsed = start.elapsed();
    ...
    report("nzbg-par2   verify", &native_times);
...
fn bench_repair(dir: &Path, iterations: u32) {
    ...
    for _ in 0..iterations {
        ...
        let start = Instant::now();
        let verify_result = nzbg_par2::verify_recovery_set(&rs, dir);
        nzbg_par2::repair_recovery_set(&rs, &verify_result, dir).expect("native repair failed");
        let elapsed = start.elapsed();
        native_times.push(elapsed);
    ...
    report("nzbg-par2   repair", &native_times);
    report_speedup(&cli_times, &native_times);
}
...
fn report_speedup(cli: &[std::time::Duration], native: &[std::time::Duration]) {
    ...
    if ratio >= 1.0 {
        println!("  => nzbg-par2 is {ratio:.2}x faster than par2cmdline");
    } else {
        println!(
            "  => nzbg-par2 is {:.2}x slower than par2cmdline",
            1.0 / ratio
        );
    }
}
