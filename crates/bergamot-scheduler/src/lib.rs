...
pub struct ServiceDeps {
    pub queue: bergamot_queue::QueueHandle,
    pub disk: std::sync::Arc<bergamot_diskstate::DiskState<bergamot_diskstate::JsonFormat>>,
...
impl Scheduler {
    pub fn new(tasks: Vec<ScheduledTask>, queue: bergamot_queue::QueueHandle) -> Self {
        Self {
            tasks,
    ...
    pub fn from_config_with_executor(
        config: &Config,
        _queue: bergamot_queue::QueueHandle,
        executor: CommandExecutor,
    ) -> anyhow::Result<Self> {
        let tasks = parse_scheduler_tasks(config.raw())?;
        Ok(Self {
...
pub struct CommandDeps {
    ...
    pub feed_handle: Option<bergamot_feed::FeedHandle>,
    pub server_pool: Option<std::sync::Arc<bergamot_nntp::ServerPoolManager>>,
}
...
impl CommandExecutor {
    pub fn new(queue: bergamot_queue::QueueHandle) -> Self {
        Self {
            queue,
...
pub struct ConnectionCleanup {
    pool: Option<std::sync::Arc<bergamot_nntp::ServerPoolManager>>,
...
impl ConnectionCleanup {
    ...
    pub fn with_pool(mut self, pool: std::sync::Arc<bergamot_nntp::ServerPoolManager>) -> Self {
        self.pool = Some(pool);
        self
    }
}
...
impl StatsTracker {
    ...
    pub fn reset_volume(&mut self, server_id: u32) {
        if server_id == 0 {
            for volume in self.volumes.values_mut() {
                *volume = ServerVolume::default();
            }
        } else if let Some(volume) = self.volumes.get_mut(&server_id) {
            *volume = ServerVolume::default();
        }
    }
}
...
impl SharedStatsTracker {
    ...
    pub fn reset_volume(&self, server_id: u32) {
...
impl bergamot_nntp::StatsRecorder for SharedStatsTracker {
    ...
    pub fn snapshot_to_queue_state(
        ...
        bergamot_diskstate::QueueState {
            ...
            nzbs: snapshot
                .nzbs
                .iter()
                .map(|nzb| bergamot_diskstate::NzbState {
                    id: nzb.id,
                    name: nzb.name.clone(),
...
<nzb xmlns="http://www.newzbin.com/DTD/2003/nzb">
  <file poster="user@example.com" date="1706140800"
    ...
    async fn scheduler_tick_updates_last_executed() {
        ...
        let (mut coordinator, handle, _rx, _rate_rx) = bergamot_queue::QueueCoordinator::new(
            2,
            1,
    ...
    async fn disk_state_flush_saves_file_article_states() {
        ...
        handle
            .report_download(bergamot_queue::DownloadResult {
                article_id: bergamot_queue::ArticleId {
                    nzb_id: assignments.0,
                    file_idx: 0,
                    seg_idx: 0,
                },
                outcome: bergamot_queue::DownloadOutcome::Success {
                    data: vec![1, 2, 3],
                    offset: 0,
                    crc: 0xDEAD,
                },
    ...
    async fn disk_state_flush_cleans_orphaned_file_states() {
        use bergamot_diskstate::{DiskState, FileArticleState, JsonFormat};
    ...
    fn snapshot_to_queue_state_maps_new_fields() {
        ...
        let snapshot = bergamot_queue::QueueSnapshot {
            nzbs: vec![bergamot_queue::NzbSnapshotEntry {
                id: 42,
                name: "test".to_string(),
                ...
                dupe_score: 99,
                dupe_mode: bergamot_core::models::DupMode::All,
                added_time: added,
                total_size: 1000,
                downloaded_size: 500,
                failed_size: 10,
                ...
                file_ids: vec![1, 2, 3],
                post_stage: None,
                post_stage_progress: 0,
                post_info_text: String::new(),
                post_stage_time_sec: 0,
                post_total_time_sec: 0,
            }],
            history: vec![],
            next_nzb_id: 43,
            next_file_id: 10,
        ...
        assert_eq!(nzb.dupe_mode, bergamot_core::models::DupMode::All);
    ...
    async fn command_executor_activate_server() {
        ...
        let server = bergamot_nntp::NewsServer {
            ...
            username: None,
            password: None,
            encryption: bergamot_nntp::Encryption::None,
            cipher: None,
            connections: 2,
            retention: 0,
            level: 0,
    ...
    async fn command_executor_deactivate_server() {
        ...
        let manager = std::sync::Arc::new(bergamot_nntp::ServerPoolManager::new(vec![server]));
        assert_eq!(manager.server_count().await, 1);
    ...
    async fn command_executor_fetch_feed_calls_handle() {
        use bergamot_feed::{FeedConfig, FeedCoordinator, FeedHandle, FeedHistoryDb};

        struct MockFetcher;
        #[async_trait::async_trait]
        impl bergamot_feed::fetch::FeedFetcher for MockFetcher {
        ...
        let (mut queue_coord, handle, _rx, _rate_rx) = bergamot_queue::QueueCoordinator::new(
            2,
            1,
            std::path::PathBuf::from("/tmp/inter"),
            std::path::PathBuf::from("/tmp/dest"),
    ...
    async fn disk_space_monitor_with_queue_builder() {
        ...
        let config = bergamot_config::Config::from_raw(std::collections::HashMap::new());
    ...
    async fn connection_cleanup_with_pool_succeeds() {
        let manager = std::sync::Arc::new(bergamot_nntp::ServerPoolManager::new(vec![]));
        let mut cleanup = ConnectionCleanup::new().with_pool(manager);
        cleanup.tick().await.expect("tick with pool should succeed");
    }

    fn sample_nzb(id: u32, name: &str) -> bergamot_core::models::NzbInfo {
        bergamot_core::models::NzbInfo {
            ...
            category: String::new(),
            priority: bergamot_core::models::Priority::Normal,
            dup_key: String::new(),
    ...
    async fn history_cleanup_deletes_old_entries() {
        ...
        let queue = bergamot_core::models::DownloadQueue {
            queue: vec![],
            history: vec![bergamot_core::models::HistoryInfo {
                id: 1,
    ...
    async fn history_cleanup_deletes_multiple_old_entries() {
        ...
        let history_entries: Vec<_> = (1..=3)
            .map(|i| bergamot_core::models::HistoryInfo {
                id: i,
                kind: bergamot_core::models::HistoryKind::Nzb,
                time: std::time::SystemTime::UNIX_EPOCH,
