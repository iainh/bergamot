...
```sh
cargo run -p bergamot-nntp-stub -- \
  --bind 127.0.0.1:3119 \
...
```json
{
  "greeting": "200 bergamot test server ready",
  "groups": {
    "alt.test": {
      "article_ids": ["1@test"],
...
```sh
cargo run -p bergamot-nntp-stub -- --require-auth --username bergamot --password test
```

### Delay and disconnect

```sh
cargo run -p bergamot-nntp-stub -- --delay-ms 250 --disconnect-after 5
```
