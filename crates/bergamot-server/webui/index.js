...
/*** WEB-INTERFACE SETTINGS (THIS IS NOT BERGAMOT CONFIG!) ***********************************/
...
var UISettings = (new function($)
{
	...
	this.description['windowTitle'] = 'Window-title for browser.\n\n' +
		...
		'Examples:\n' +
		' "%(COUNT-)% Bergamot" - show number of downloads in parenthesis followed by a hyphen; don\'t show "(0) - " if queue is empty;\n' +
		' "%PAUSE% %(COUNT-)% Bergamot" - as above but also show pause-indicator if paused;\n' +
		' "%[COUNT]% %SPEED-% Bergamot" - show number of downloads and speed if not null (default setting).';
	this.windowTitle = '%[COUNT]% %SPEED-% Bergamot';
	...
	// URL for communication with Bergamot via JSON-RPC
	this.rpcUrl = './jsonrpc';
...
var Refresher = (new function($)
{
	...
	this.init = function()
	{
		...
		RPC.connectErrorMessage = 'Cannot establish connection to Bergamot.'
