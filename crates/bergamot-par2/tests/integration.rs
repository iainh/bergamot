...
fn end_to_end_with_par2_cli() {
    ...
    assert!(
        matches!(
            f1_result.status,
            bergamot_par2::FileVerifyStatus::Damaged { .. }
        ),
    ...
    let f2_result = result
        ...
        .unwrap();
    assert_eq!(f2_result.status, bergamot_par2::FileVerifyStatus::Missing);
}

#[test]
fn end_to_end_synthetic_par2() {
    use bergamot_par2::format::{HEADER_SIZE, MAGIC};
    ...
    // Creator packet
    let creator_pkt = make_packet(&set_id, b"PAR 2.0\0Creator\0", b"bergamot-par2-test\0\0");
